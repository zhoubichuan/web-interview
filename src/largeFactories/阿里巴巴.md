...
- 组件 dispatch action。
- middleware 可处理异步、日志等副作用。
- reducer 根据旧 state 和 action 返回新 state。
- store 通知订阅者，组件拿到新状态后重新渲染。
...
- 物理像素和 CSS 像素在高 DPR 屏上不一致，`1px` CSS 可能看起来偏粗。
- 可用 `transform: scaleY(0.5)`、伪元素缩放、viewport 缩放或 border-image 等方案。

## 20.介绍 flex 布局

- Flex 是一维布局模型，通过主轴和交叉轴控制子元素排列。
- 常用属性有 `display:flex`、`flex-direction`、`justify-content`、`align-items`、`flex`。
- 适合居中、等分、两端对齐和自适应布局。
...
- webpack plugin 是带 `apply(compiler)` 方法的对象。
- 插件通过 Tapable hooks 监听编译生命周期。
- 在不同阶段读取或修改 compilation、assets、chunks 等数据。

## 25.dev-server 是怎么跑起来

- webpack-dev-server 启动本地 HTTP 服务，并在内存中编译资源。
- 通过 webpack watch 监听文件变化。
- 配合 websocket 通知浏览器刷新或执行 HMR。
...
- webpack 4/5 使用 `optimization.splitChunks` 抽取公共模块。
- 可按 vendors、common、运行时代码分别拆分。
- 旧版本常用 `CommonsChunkPlugin`。
...
- 深拷贝需要递归复制对象自身属性，并处理数组、日期、正则等类型。
- 要注意循环引用，可用 `WeakMap` 缓存已拷贝对象。
- 现代浏览器可使用 `structuredClone`，工程中常用 `lodash.cloneDeep`。
