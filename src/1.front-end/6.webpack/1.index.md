...
- HappyPack 会把耗时 loader 分发到多个子进程并行处理。
- 适合旧版本 webpack 中 Babel 等 CPU 密集型 loader 优化；webpack 5 项目可优先考虑 thread-loader 或持久缓存。

#### 3.DllPulgin:可以将特定的类库提前打包然后引入

- DLLPlugin 可以把 React、Vue 等稳定依赖提前打成动态链接库。
- 业务构建时通过 DllReferencePlugin 引用，减少重复编译时间。

#### 4.代码压缩：mode 为 production 默认开启代码压缩

- webpack production 模式默认启用压缩和部分优化。
- JS 常用 Terser 压缩，CSS 可配合 css-minimizer-webpack-plugin。

#### 5.tree shaking：删除项目中未被引用的代码

- tree shaking 依赖 ES Module 的静态结构分析未使用导出。
- 生产模式下结合压缩工具删除 dead code，库代码需要正确配置 `sideEffects`。
