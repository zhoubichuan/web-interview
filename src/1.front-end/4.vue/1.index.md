...
- `mixins` 是组件选项，接收一个 mixin 数组，用来把多个混入对象合并到组件中。
- `mixin` 通常指单个混入对象；全局 `Vue.mixin()` 会影响所有组件，应谨慎使用。
...
原理：新旧节点对比，尽可能使用旧节点，多的节点删除，少的节点创建

- vue2 中循环遍历新旧节点，比较 `头头`、`尾尾`、`头尾`、`尾头`、`中间`节点通过映射处理，相同保留移动，少的创建多的删除
- vue3 中也是循环遍历新旧节点，比较`头头`、`尾尾`、`中间`节点寻找最大最长递增子序列，对乱序部分计算最少移动方案
